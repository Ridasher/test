...
export default function MessageDrafts() {
  ...
  const loadDrafts = async () => {
    const items = await base44.entities.MessageDraft.list("-created_date", 30);
    setDrafts(items);
  };
  ...
  const handleGenerate = async () => {
    ...
    const prompt = `Tu es un expert en communication professionnelle sur LinkedIn, en français.
Rédige un message LinkedIn personnalisé avec les paramètres suivants :

Destinataire : ${recipientName}
Type de message : ${contextLabels[context]}
${contextDetails ? `Contexte additionnel : ${contextDetails}` : ""}

Le message doit :
- Être professionnel mais chaleureux
- Être personnalisé avec le nom du destinataire
- Faire entre 50 et 150 mots
- Avoir une ouverture engageante
- Terminer par une question ou une invitation à poursuivre l'échange
- Ne pas être trop formel ni trop familier

Retourne uniquement le texte du message, prêt à être copié-collé dans LinkedIn.`;
  ...
  const handleSaveDraft = async () => {
    ...
    await base44.entities.MessageDraft.create({
  ...
  const handleDeleteDraft = async (id) => {
    await base44.entities.MessageDraft.delete(id);
    loadDrafts();
  };

  const handleUpdateStatus = async (id, status) => {
    await base44.entities.MessageDraft.update(id, { status });
    loadDrafts();
  };
        ...
        {loading && (
          <div className="flex items-center justify-center py-10">
            <div className="text-center space-y-3">
              ...
              <p className="text-sm text-slate-500">L'IA rédige votre message...</p>
            </div>
          </div>
        )}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-medium text-slate-800 text-sm">{draft.recipient_name}</span>
                  ...
                  <p className="text-xs text-slate-400">
                    {new Date(draft.created_date).toLocaleDateString("fr-FR", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                      hour: "2-digit",
